...
class Vocabulary:
    ...
    def get_subset_by_freq(self, min_freq):
        tokens = sorted(
            list(self.token2index.keys()),
            key=lambda token: self.token_counts[self[token]],
            reverse=True
        ...
        for index, token in enumerate(tokens):
            curr_count = self.token_counts[self.token2index[token]]
            if curr_count >= min_freq:
                token2index_subset[token] = index
                index2token_subset[index] = token
                token_counts_subset.append(curr_count)

        return type(self)(
            token2index=token2index_subset,
            index2token=index2token_subset,
            token_counts=token_counts_subset
        )
